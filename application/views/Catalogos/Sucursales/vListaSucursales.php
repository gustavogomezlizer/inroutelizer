...
$editar=VERIFICARPERFILFUNCION("Catalogos","editaSucursal",$this->session->userdata('perfil'));
$nuevo=VERIFICARPERFILFUNCION("Catalogos","nuevaSucursal",$this->session->userdata('perfil'));
?>
		...
		<div class="page-content">					
			<div class="page-header">
				<h1>
					...
					<small><i class="ace-icon fa fa-angle-double-right"></i>Catalogos / Sucursales</small>
				</h1>
			</div><!-- /.page-header -->
				...
				<div class="col-xs-12">								
					<div class="row"><!--  empieza div.row de la tabla clientes -->
						<div class="col-xs-12">	<!--  empieza div.col-xs-12 de la tabla clientes -->										
							...
							<table id="tabla_sucursales" class="table table-striped table-bordered table-hover">
								<thead>
									<tr>
										<th>Clave</th>
										<th>Sucursal</th>
								...
								<tbody>																									
													...
													/*foreach ($lista->result() as $kLC) {
														?>
													<tr>
														<td><?php echo $kLC->clave; ?></td>
														<td><?php echo $kLC->sucursal; ?></td>
														<td><?php echo $kLC->direccion; ?></td>
														<td><?php echo $kLC->ciudad; ?></td>
														<td>
															<?php echo $kLC->descripcion;
															 ?>
														...
														$textoProveedores=""; 
															foreach ($listaProveedores->result() as $kProv) {
																...
																$valProCli=$this->CatalogosModel->validarProveedorSucursal($kProv->id,$kLC->id);
																if($valProCli=="SI"){
																	if($textoProveedores==""){
																						$textoProveedores=$kProv->nombre;
																					}
																					else{
																						$textoProveedores.=", ".$kProv->nombre;
																					}
														...
														<td><div class="hidden-sm hidden-xs action-buttons">
																<a id="VER1<?php echo $kLC->id; ?>" class="blue verSucursal1" href="#">
																	<i class="ace-icon fa fa-eye bigger-130"></i>
																</a>
																<?php if($editar==1){ ?>
																<a id="EDIT1<?php echo $kLC->id; ?>" class="green editarSucursal1" href="#">
															...
															<div class="hidden-md hidden-lg">
																<div class="inline pos-rel">
																	...
																	<ul class="dropdown-menu dropdown-only-icon dropdown-yellow dropdown-menu-right dropdown-caret dropdown-close">
																		<li>
																			<a id="VER2<?php echo $kLC->id; ?>" href="#" class="tooltip-info verSucursal1" data-rel="tooltip" title="Ver">
																				<span class="blue">
																		...
																		<?php if($editar==1){ ?>
																		<li>
																			<a id="EDIT2<?php echo $kLC->id; ?>" href="#" class="tooltip-success editarSucursal1" data-rel="tooltip" title="Editar">
																				<span class="green">
...
<script>
	...
	var i_clave=0, i_sucursal=1, i_domicilio=2, i_ciudad=3, i_descripcion=4, i_proveedores=5, i_activo=6, i_acciones=7;

	var myTable = $('#tabla_sucursales').DataTable({
		"language": {
	...
	$(".btnAgregar").click(function(event) {					
		var link = "<?php echo LINKPROYECTO('NuevoSucursal'); ?>";
	...
	{
		$('#tabla_sucursales').addClass('loadingtable');
		$('#tabla_sucursales tbody').html("");

		$.post("<?php echo LINKPROYECTO('ListadoSucursalesJson') ?>", function(data)
		{
			var datos = JSON.parse(data);
			if(datos.length > 0)
			{
				myTable.destroy();
				myTable = $('#tabla_sucursales').DataTable({
					"language": {
				...
				if(CARGAR_BOTONES_TABLA=="0")
				{
					...
					$('#tabla_sucursales tbody').on( 'click', 'button.editrow', function () {
						var row = myTable.row( $(this).parents('tr') ).data();								
						window.location.href = "<?php echo LINKPROYECTO('EditarSucursal/'); ?>" + row.id;
					});
					$('#tabla_sucursales tbody').on( 'click', 'button.showrow', function () {
						var row = myTable.row( $(this).parents('tr') ).data();
						window.location.href = "<?php echo LINKPROYECTO('VerSucursal/'); ?>" + row.id;
					});
			...
			$('#tabla_sucursales').removeClass('loadingtable');
		});
	}
