...
<script>
	...
	$(".btnAgregar").click(function(event) {					
		var link = "<?php echo LINKPROYECTO('NuevoMarca'); ?>";
	...
	function cargarTablaProductos()
	{
		...
		$.post("<?php echo LINKPROYECTO('ListadoMarcasJson') ?>", function(data)
		{
			...
			if(datos.length > 0)
			{
					...
					"columnDefs": [
						...
						},
						{
                            "render": function ( data, type, row ) {
                                return (row.estatus==1) ? "<span class='label label-sm label-success'>SI</span>" : "<span class='label label-sm label-danger'>NO</span>";
                            },
                            "targets": i_activo,
                        }						
					]
				...
				if(CARGAR_BOTONES_TABLA=="0")
				{
					...
					$('#tabla_categorias tbody').on( 'click', 'button.editrow', function () {
						...
						window.location.href = "<?php echo LINKPROYECTO('EditarMarca/'); ?>" + row.id;
					});
					$('#tabla_categorias tbody').on( 'click', 'button.showrow', function () {
						var row = myTable.row( $(this).parents('tr') ).data();
						window.location.href = "<?php echo LINKPROYECTO('VerMarca/'); ?>" + row.id;
					});
