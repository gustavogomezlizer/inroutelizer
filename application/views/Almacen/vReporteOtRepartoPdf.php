...
<html5 lang="es">
	...
	<body>
		...
		<?php 
			...
			{
				if(!in_array($pedido["id_usuario_reparto"], $items_rutas))
				{
					array_push($items_rutas, $pedido["id_usuario_reparto"]);
				}
			}
			...
			/*foreach($items_rutas as $ruta) 
			{
				...
				$detalle = array_filter($info_pedidos, function ($var) use($ruta) {
					return ($var['id_usuario_reparto'] == $ruta);
				});
		...
		<?php foreach($items_rutas as $ruta) { ?>
			...
			<div class="principales">
				...
				{
					if(!in_array($pedido["tipo"], $items_tipo))
					{
						array_push($items_tipo, $pedido["tipo"]);
					}
				}
				?>
				...
				<main>
					...
					<table width="30%" border=1 cellspacing=0>
						<tr>
							...
							<td><?php echo $detalle[0]["rutas_nombres"]; ?></td>
						</tr>
						<tr>
					...
					<table width="100%" border=1 cellspacing=0>
						<thead>
							<tr class="table_encabezado">
								...
								<th width="7%" align="right">Suma Total</th>
							</tr>
						</thead>
						...
						<tbody>
							...
							foreach($items_tipo as $tipo) 
							{ 
								$detalle_tipo = array_filter($detalle, function ($var) use($tipo) {
									return ($var['tipo'] == $tipo);
								});
				
								$detalle_tipo = array_values(array_filter($detalle_tipo));

								array_multisort(
									array_column($detalle_tipo, 'categoria_nombre'),  SORT_ASC,
									array_column($detalle_tipo, 'producto'), SORT_ASC,
									$detalle_tipo
								);
							?>
								...
								<?php foreach($detalle_tipo as $pedido) { ?>
									...
									?>
										...
										<tr>
											...
											<td class="<?php echo $colorpaquete; ?>"><?php echo $pedido["categoria_nombre"]; ?></td>
								...
								<tr class="table_totales">
									...
									<td><?php echo ""; ?></td>
									<td><?php echo ""; ?></td>
									<td align="right"><?php echo "Total $tipo"; ?></td>
									<td align="right"><?php echo ""; ?></td>
