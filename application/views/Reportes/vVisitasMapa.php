...
<script type="text/javascript">
	...
	$("#btnAplicar").on("click", function(){
		...
		$.post("<?php echo CREPORTES('listadoVisitasJson');?>", {fechade: fecha, fechaa: fecha, ruta: idruta, sucursal: 0, usuario: 0},function(data){
			...
			if(datos.length > 0)
			{
				...
				for(var x in datos)
				{
					...
					//iconColor.options.number = datos[x].ruta_nombre.slice(-2);

					marker = new L.marker([datos[x].latitud, datos[x].longitud], {icon: iconColor})
					.bindPopup(
						datos[x].cliente + "<br/><br/>" + datos[x].domicilio
					)
					.addTo(map);
