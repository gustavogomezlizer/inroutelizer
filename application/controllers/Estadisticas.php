...
class Estadisticas extends CI_Controller {
	public function __construct()
	{
		...
		$this->load->model(array('EstadisticasModel','HomeModel','CatalogosModel','ConfigurarModel','ReportesModel'));
	}
	/**
	...
	public function ObjetivosAcumulados()
	{
		VERIFICARSESION();
		...
		$data["listaCategorias"] = $this->EstadisticasModel->getCategorias();
		$data["listaSucursales"] = $this->ReportesModel->getSucursales();
		$this->load->view('Estadisticas/vObjetivosAcumulados', $data);
	}

	public function getListaAcumuladosJson()
	{
		$data = $this->input->post();
		//$data = array("categoria"=>"0");
		echo json_encode($this->EstadisticasModel->getObjetivosAcumulados($data));
	}

	public function ProyeccionNomina()
	{
		VERIFICARSESION();
		...
		$data["listaCategorias"] = $this->EstadisticasModel->getCategorias();
		$data["listaSucursales"] = $this->ReportesModel->getSucursales();
		$this->load->view('Estadisticas/vProyeccionNomina', $data);
	}

	public function ProyeccionNomina2()
	{
		VERIFICARSESION();
		...
		$data["listaCategorias"] = $this->EstadisticasModel->getCategorias();
		$data["listaSucursales"] = $this->ReportesModel->getSucursales();
		$this->load->view('Estadisticas/vProyeccionNomina2', $data);
	}

	public function getListaProyeccionNominaJson()
	{
		$data = $this->input->post();
		//$data = array("categoria"=>"0");
		echo json_encode($this->EstadisticasModel->getProyeccionNomina($data));
	}

	public function getListaProyeccionNominaJson2()
	{
		$data = $this->input->post();
		echo json_encode($this->EstadisticasModel->getProyeccionNomina2($data));
	}

	public function getProyeccionNominaCertificado()
	{
		$data = $this->input->post();
		echo json_encode($this->EstadisticasModel->getProyeccionNominaCertificado($data));
	}

	public function proyeccionNominaPreventasPdf($idruta, $periodo, $idsucursal)
	{
		...
		$data["lista_categorias_proyeccion"] = $this->EstadisticasModel->getProyeccionNomina2($post);
		$data["certificado"] = $this->EstadisticasModel->getProyeccionNominaCertificado($post);

		$html = $this->load->view("Estadisticas/vProyeccionNominaPdf", $data, TRUE);
	...
	{
		//$cadena='{"TipoArchivo":"Objetivos","Periodo":"201811","Objetivos":[{"IdVendedor":"120","ObjetivoCat":[{"Categoria":"ALIMENTOS INFANTILES","Importe":"2844.40"},{"Categoria":"BEBIDAS COCOA","Importe":"316.20"}]},{"IdVendedor":"14","ObjetivoCat":[{"Categoria":"ALIMENTOS INFANTILES","Importe":"10866.80"},{"Categoria":"BEBIDAS COCOA","Importe":"3576.20"}]},{"IdVendedor":"77","ObjetivoCat":[{"Categoria":"LACTEOS INFANTILES","Importe":"56.80"},{"Categoria":"LACTEOS POLVO","Importe":"2933.90"}]}]}';
		$cadena=$_POST['cadena'];
		//$cadena="jajaja";
		//echo $cadena;
		$res=$this->ReportesModel->getAgregarObjetivos($cadena);
		//$res=$this->ReportesModel->getAgregarAcumuladosx($cadena);
	
		//print_r($cadena);
	}*/

	public function pruebaLeerObjetivos(){
		$this->load->view('pruebapost');
	}
	public function leerPruebaObjetivos(){
		$res=$this->ReportesModel->getLeerPrueba();
		echo $res;
	}
	public function pruebaNewModulo(){
		//$cadena="Hola";
		$cadena=$_REQUEST['nombre']."-".$_REQUEST['clave'];
		$res=$this->ReportesModel->getAgregarAcumuladosx($cadena);
		echo $cadena;
	}
	

	public function leerObjetivosVentasPrueba(){
	...
	public function listadoObjetivosAcu($fIni="1900-01-01",$fFin="1900-01-01",$tipo="TODOS",$usuario="TODOS",$sucursal="TODOS",$ruta="TODOS"){
		VERIFICARSESION();
...
public function imprimirPedido2(){
	...
	public function ObjetivosAcumulados2($periodo="190001",$categoria="TODOS",$ruta="TODOS",$sucursal="TODOS"){
		VERIFICARSESION();
		date_default_timezone_set('America/Mazatlan');
		$fecha1=date('Y-m-d');	
		if($periodo=="190001"){
		...
		$data["ruta"]=$ruta;
		$data["lista"]=$this->EstadisticasModel->getObjetivosAcumulados($periodo);
		$data["listaCategorias"]=$this->EstadisticasModel->getCategorias();
		$data["listaSucursales"]=$this->ReportesModel->getSucursales();
		$data["listaRutas"]=$this->ReportesModel->getRutasPedidos();
		//echo $data["lista"];
		$this->load->view('Estadisticas/vObjetivosAcumulados2',$data);
